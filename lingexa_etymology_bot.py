"""
Lingexa Etymology - English Word Origins & History
Discover the fascinating origins of English words
"""
...
def generate_word_data(num_words: int = WORDS_PER_VIDEO) -> list:
    max_attempts = 30
    categories = [
        "ancient origins and ancient history",
        "Latin influences on English",
        "Greek roots in modern English",
        "French loanwords in English",
        "Germanic and Norse etymology",
        "words from Arabic origins",
        "Italian contributions to English",
        "Sanskrit and Hindi loanwords",
        "Celtic and Gaelic word origins",
        "Spanish influences on English",
        "Dutch and Flemish loanwords",
        "words from Persian origins",
        "African language contributions",
        "Chinese and Japanese loanwords",
        "Native American word origins",
        "Russian and Slavic loanwords",
        "invented words and neologisms",
        "words from mythology and religion",
        "scientific terms from Greek/Latin",
        "words from人名 and place names",
    ]
    collected = []
    collected_lower = set()
...
def create_background():
    ...
    for y in range(VIDEO_HEIGHT):
        ...
        if ratio < 0.5:
            ...
            r = int(240 + (232 - 240) * ((ratio - 0.5) * 2))
            g = int(237 + (228 - 237) * ((ratio - 0.5) * 2))
            b = int(230 + (222 - 230) * ((ratio - 0.5) * 2))
        draw.rectangle([(0, y), (VIDEO_WIDTH, y + 1)], fill=(r, g, b))
    accent = Image.new('RGBA', (VIDEO_WIDTH, VIDEO_HEIGHT), (0, 0, 0, 0))
    accent_draw = ImageDraw.Draw(accent)
    for i in range(0, VIDEO_WIDTH, 160):
        for j in range(0, VIDEO_HEIGHT, 160):
            accent_draw.ellipse(
                [(i + 40, j + 40), (i + 90, j + 90)],
                outline=(200, 180, 150, 15),
                width=1
            )
...
def generate_all_audio(words: list, output_dir: str):
    ...
    for i, word_data in enumerate(words):
        ...
        century = word_data.get("century", "")
        text = f"{word}. {pos}. {definition}. Example: {example}."
        if origin:
            text += f" Origin: {origin}."
        if century:
            text += f" Entered English in the {century}."
...
def generate_word_image(word_data: dict, bg_image, output_path: str):
    ...
    font_word = load_font(fonts_bold, 150)
    font_level = load_font(fonts_bold, 50)
    font_pos = load_font(fonts_bold, 60)
    font_def_label = load_font(fonts_bold, 42)
    font_def = load_font(fonts_regular, 65)
    font_ex_label = load_font(fonts_bold, 42)
    font_ex = load_font(fonts_regular, 54)
    font_origin_label = load_font(fonts_bold, 42)
    font_origin = load_font(fonts_regular, 50)
    ...
    draw.rectangle([(0, 0), (VIDEO_WIDTH, 90)], fill=(25, 30, 60))
    draw.text((CENTER_X, 45), CHANNEL_NAME.upper(), fill=(255, 200, 80), font=font_header, anchor="mm")
    ...
    draw.text((CENTER_X, y_cursor), word, fill=(25, 30, 60), font=word_font, anchor="mm", stroke_width=max(1, word_font_size // 40), stroke_fill=(220, 215, 200))
    y_cursor += word_h + 50

    if language:
        language_text = language.upper()
        level_bbox = draw.textbbox((0, 0), language_text, font=font_level)
        level_w = level_bbox[2] - level_bbox[0]
        level_h = level_bbox[3] - level_bbox[1]
        draw.rounded_rectangle(
            [(CENTER_X - level_w // 2 - 14, y_cursor), (CENTER_X + level_w // 2 + 14, y_cursor + level_h + 20)],
            radius=10, fill=(200, 160, 50)
        )
        draw.rounded_rectangle(
            [(CENTER_X - level_w // 2 - 14, y_cursor), (CENTER_X + level_w // 2 + 14, y_cursor + level_h + 20)],
            radius=10, fill=None, outline=(255, 200, 80), width=2
        )
        draw.text((CENTER_X, y_cursor + level_h // 2 + 10), language_text, fill=(25, 30, 60), font=font_level, anchor="mm")
        y_cursor += level_h + 50
        if century:
            century_text = f"({century})"
            century_font = load_font(fonts_regular, 36)
            draw.text((CENTER_X, y_cursor), century_text, fill=(160, 155, 170), font=century_font, anchor="mm")
            y_cursor += 50
        else:
            y_cursor += 15
    ...
    if pos:
        ...
        draw.text((CENTER_X, y_cursor + pos_h // 2 + 10), pos_text, fill=(255, 200, 80), font=font_pos, anchor="mm")
        y_cursor += pos_h + 70

    def_label = "MEANING"
    draw.text((MARGIN_X, y_cursor), def_label, fill=(25, 30, 60), font=font_def_label, anchor="lm")
    y_cursor += 60
    ...
    def_box = Image.new('RGBA', (CONTENT_WIDTH, def_box_h), (25, 30, 60, 255))
    def_draw = ImageDraw.Draw(def_box)
    def_draw.rounded_rectangle([(0, 0), (CONTENT_WIDTH, def_box_h)], radius=18, fill=(25, 30, 60, 255))
    ...
    ex_label = "EXAMPLE"
    draw.text((MARGIN_X, y_cursor), ex_label, fill=(25, 30, 60), font=font_ex_label, anchor="lm")
    y_cursor += 60
    ...
    ex_box = Image.new('RGBA', (CONTENT_WIDTH, ex_box_h), (55, 60, 95, 220))
    ex_draw = ImageDraw.Draw(ex_box)
    ex_draw.rounded_rectangle([(0, 0), (CONTENT_WIDTH, ex_box_h)], radius=15, fill=(55, 60, 95, 220))
    ...
    if origin:
        ...
        origin_box = Image.new('RGBA', (CONTENT_WIDTH, origin_box_h), (25, 30, 60, 240))
        origin_draw = ImageDraw.Draw(origin_box)
        origin_draw.rounded_rectangle([(0, 0), (CONTENT_WIDTH, origin_box_h)], radius=16, fill=(25, 30, 60, 240))
        accent_bar = Image.new('RGBA', (8, origin_box_h), (255, 200, 80, 255))
        origin_box.paste(accent_bar, (0, 0), accent_bar)
        for i, line in enumerate(origin_lines):
            line_y = origin_padding + (i * origin_line_spacing) + (origin_line_height // 2)
            origin_draw.text((CONTENT_WIDTH // 2, line_y), line, fill=(255, 220, 120), font=font_origin, anchor="mm")
        img.paste(origin_box, (MARGIN_X, y_cursor), origin_box)

    draw.rectangle([(0, VIDEO_HEIGHT - 65), (VIDEO_WIDTH, VIDEO_HEIGHT)], fill=(25, 30, 60))
    draw.text((CENTER_X, VIDEO_HEIGHT - 32), f"Word Origins Daily  |  {CHANNEL_NAME}", fill=(255, 200, 80), font=font_footer, anchor="mm")
...
def generate_reel():
    print(f"\n{'='*80}\n  {CHANNEL_NAME.upper()} - ENGLISH WORD ORIGINS\n{'='*80}\n")
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    reel_dir = VIDEO_DIR / f"words_{timestamp}"
    reel_dir.mkdir(exist_ok=True)
    print("[1/5] Generating 5 words with etymology...")
    words = generate_word_data(WORDS_PER_VIDEO)
    print(f"\n  Words:")
    for i, w in enumerate(words, 1):
        print(f"    {i}. {w['word']} ({w['part_of_speech']}) - Origin: {w.get('century', 'unknown')}")
...
if __name__ == "__main__":
    ...
    print("="*80)
    print("\n  One word per screen | Rich etymology | Warm parchment aesthetic")
    print("="*80)
    generate_reel()
